...
**Fixes:**
- Use `--extract-computed` to capture JS-applied styles
- Check if stylesheets are on different domains (CORS restriction)
- Try with `--full-page` to ensure all lazy-loaded styles are captured
...
**Fixes:**
- Page readiness module handles most cases automatically: `node src/core/page-prep/page-readiness.js --url "URL"`
- For SPAs: framework detection identifies routing to wait for client-side render
...
**Fixes:**
- Asset downloader includes exponential backoff by default
- Reduce concurrency if still hitting limits
...
**Fixes:**
- Use `--aggressive-filter` flag for aggressive dead code removal
- `css-chunker.js` (v3.0) automatically streams large files in chunks
...
- JS-only hover effects (mouseenter event handlers) are detected via DOM-based method
...
| `CSS_CORS_BLOCKED` | Cross-origin stylesheet blocked | Use `--extract-computed` for JS styles |
...
| `BROWSER_LAUNCH_FAILED` | Playwright can't start browser | Run `npx playwright install chromium` |
...
- **`--dry-run`** - Preview page discovery without capturing (test URLs first)
- **Progress output** - TTY-aware progress in stderr; pipe stdout for clean output
- **`quality-score.json`** - Check output diagnostics after capture
- **`audit-report.md`** - Review detailed verification results
- **Node debug** - Run with `NODE_DEBUG=design-clone` for verbose logging
